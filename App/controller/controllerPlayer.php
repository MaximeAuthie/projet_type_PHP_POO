    ...
    class ControllerPlayer extends ManagerJoueur {
        ...
        public function showAccount(){
            ...
            $this->setListPerso($charactersList);
             ...
             //On créé la variable qui va contenir la chaine de caractère a afficher
             $listCharacters = '';

             // $arrayCharacters = $this->getListPerso();
                     foreach ($this->getListPerso() as $item) {
                         $characterClass = $item->getClasse();
                         $listCharacters= $listCharacters.'<li class="list-group-item" id="'.$item->getId().'">'.$item->getNom().' ('.$characterClass->getNom().')</li>';
                     }
        ...
        public function showAllCharacters(){
            ...
            //On exécute une boucle pour remplir le template HTML des cards avec les data de chaque occurence renvoyée par $this->getListPerso()
            foreach ($this->getListPerso() as $item) {
                $characterClass = $item->getClasse();
                $listCharacters= $listCharacters.'<div class="card w-50 mx-auto mb-5">
                            ...
                            <div class="card-body">
                                ...
                                <div class="w-25 mx-auto d-grid text-center mb-5">
                                    <a class="nav-link" href="updateCharacter/'.$item->getId().'"><button class="btn btn-primary" type="button">Modifier</button></a>
                                </div>
                            </div>
                        </div>';
            }
